...
export function AddCoachForm({
  ...
  async function action(fd: FormData) {
    const res = await createCoachAction(tournamentId, teamId, fd);
    if (res?.error) toast.error(res.error);
    else {
      const msg = matched ? `Added ${matched.fullName} as coach` : 'Coach added';
      toast.success(msg);
  ...
  async function onMobileBlur(e: React.FocusEvent<HTMLInputElement>) {
    ...
    if ('found' in res && res.found) {
      ...
      toast.info(`Found existing person: ${res.person.fullName}`);
    } else {
      setMatched(null);
    }
  }
  ...
  return (
    <form ref={formRef} action={action} className="space-y-3">
      ...
      <div className="space-y-2">
        <Label htmlFor="coach-mobile" className="text-xs">
        ...
        {matched ? (
          <div className="rounded-md border border-emerald-500/30 bg-emerald-500/5 p-2 text-xs">
            ...
            <p className="mt-1 text-[10px] text-muted-foreground">
              We&apos;ll attach this person to your team as a coach.
            </p>
          </div>
        ) : (
          <p className="text-[10px] text-muted-foreground">
            One mobile = one human. A coach who also plays reuses the same person record.
          </p>
        )}
      </div>

      <div className="space-y-2">
        <Label htmlFor="coach-fullName" className="text-xs">
          Full name <span className="text-destructive">*</span>
        </Label>
        <Input ref={nameRef} id="coach-fullName" name="fullName" placeholder="Ravi Shastri" required />
      </div>

      <div className="space-y-2">
        <Label htmlFor="coach-role" className="text-xs">
          Coaching role
        </Label>
        <Select id="coach-role" name="role" defaultValue="head_coach">
      ...
      >
        <summary className="flex cursor-pointer items-center gap-2 px-3 py-2 text-sm font-medium">
          ...
          <span className="text-xs font-normal text-muted-foreground">
            {matched?.hasPan || matched?.hasAadhaar ? '· already on file' : '· optional, sensitive'}
          </span>
        </summary>
        <div className="space-y-3 px-3 pb-3">
          <p className="text-[11px] leading-snug text-muted-foreground">
            PAN and Aadhaar are stored on the <strong>person</strong>, not this roster — fill once,
            reused everywhere. Organiser-level only, never shown publicly.
          </p>
          <div className="space-y-2">
            <Label htmlFor="coach-pan" className="text-xs">
          ...
          <div className="space-y-2">
            <Label htmlFor="coach-aadhaar" className="text-xs">
              Aadhaar number{' '}
